...
function parseDisplayYear(value: string | null) {
...
function RegulationsContent() {
  ...
  const { data, isLoading, error } = useAllDocumentsByType("regulation");

  const currentRegulationsUrl = useMemo(() => {
    const params = searchParams.toString();
    return params ? `/legislation/regulations?${params}` : "/legislation/regulations";
  }, [searchParams]);
  ...
  const updateParams = (updates: Record<string, string | undefined>) => {
    ...
    router.push(next ? `/legislation/regulations?${next}` : "/legislation/regulations");
  };
  ...
  const totalPages = Math.max(Math.ceil(regulations.length / DEFAULT_PAGE_SIZE), 1);
  ...
  const activeFilters = [
    ...
    year !== "all"
      ? { key: "year", label: yearBuckets.find((bucket) => bucket.value === year)?.label || year }
      : null,
    query ? { key: "q", label: `Search: ${query}` } : null,
  ].filter(Boolean) as Array<{ key: string; label: string }>;

  const resultLabel = `${regulations.length} regulation${regulations.length === 1 ? "" : "s"}`;
  ...
  return (
    <div className="container mx-auto max-w-6xl px-4 py-6">
      ...
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-2xl">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Search, filter, and review Uganda&apos;s statutory instruments and subsidiary legislation with clearer navigation.
          </p>
        </div>

        <div className="rounded-2xl border bg-muted/20 px-4 py-3 text-sm lg:min-w-[190px]">
          <p className="font-medium">{data?.length ?? "..."} total regulations indexed</p>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  placeholder="Title, gazette number, chapter..."
                  className="pl-9"
                />
              </div>
            </div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            {activeFilters.length > 0 && (
              <Button
                ...
                onClick={() => {
                  setSearchInput("");
                  router.push("/legislation/regulations");
                }}
              >
                Reset all
              </Button>
      ...
      <div className="mb-4 flex items-center justify-between text-sm text-muted-foreground">
        <p>
          Showing {regulations.length === 0 ? 0 : (safePage - 1) * DEFAULT_PAGE_SIZE + 1}
          {"-"}
          {Math.min(safePage * DEFAULT_PAGE_SIZE, regulations.length)} of {regulations.length}
        </p>
      ...
      {error && (
        <Card className="border-destructive">
          <CardContent className="pt-6">
            <p className="text-sm text-destructive">Failed to load regulations. Please try again.</p>
          </CardContent>
        </Card>
      )}
      ...
      {!isLoading && !error && regulations.length === 0 && (
        <Card>
          <CardContent className="py-14 text-center">
            ...
            <h3 className="mt-4 text-lg font-medium">No regulations match these filters</h3>
            <p className="mt-2 text-sm text-muted-foreground">
              Adjust the search, clear a filter, or browse all regulations again.
            </p>
      ...
      {!isLoading && !error && paginatedRegulations.length > 0 && (
        <div className={cn(view === "grid" ? "grid gap-4 sm:grid-cols-2 xl:grid-cols-3" : "space-y-3")}>
          {paginatedRegulations.map((regulation) => {
            const displayYear = resolveDocumentYear(regulation);
            const documentHref = `/document/${regulation.id}?returnTo=${encodeURIComponent(currentRegulationsUrl)}&from=regulations`;
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 space-y-2">
                        ...
                        {regulation.short_title && regulation.short_title !== regulation.title && (
                          <p className="text-sm text-muted-foreground">{regulation.short_title}</p>
                        )}
                      </div>
                  ...
                  <CardContent className={cn("space-y-3", view === "grid" ? "pt-0 pb-4" : "pb-3 pt-0")}>
                    ...
                    <div className="flex items-center justify-between gap-3 text-sm">
                      <span className="text-muted-foreground">
                        Open full regulation, contents, and metadata
                      </span>
